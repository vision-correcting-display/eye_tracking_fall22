...
class SingleModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        size = opt.fineSize
        self.opt = opt
        self.input_A = torch.FloatTensor(nb, opt.input_nc, size, size)
        self.input_B = torch.FloatTensor(nb, opt.output_nc, size, size)
        self.input_img = torch.FloatTensor(nb, opt.input_nc, size, size)
        self.input_A_gray = torch.FloatTensor(nb, 1, size, size)
    ...
    def test(self):
        ...
        if self.opt.noise > 0:
            self.noise = Variable(torch.Tensor(self.real_A.size()).normal_(mean=0, std=self.opt.noise/255.))
